...
TBinaryTreeItem* copy (TBinaryTreeItem* root) {
    ...
    TBinaryTreeItem* root_copy = new TBinaryTreeItem (root->GetTriangle());
...
void TBinaryTree::Push (Triangle &triangle) {
    if (root == NULL) {
    root = new TBinaryTreeItem(triangle);
    }
        ...
        current = new TBinaryTreeItem(triangle);
        if (childInLeft == true) {
...
TBinaryTreeItem* TBinaryTree:: Pop(TBinaryTreeItem* root, Triangle &triangle) {
    if (root == NULL) {
        return root;
    }
...
Triangle& TBinaryTree::GetItemNotLess(double area, TBinaryTreeItem* root) {
